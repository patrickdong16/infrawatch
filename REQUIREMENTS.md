...
### 4.4 Phase 4 (v1.3 新增功能)

| 模块 | 优先级 | 功能描述 | 状态 |
|------|--------|----------|------|
| **P11 收入vs成本趋势对比** | P0 | 挣钱速度vs花钱速度直接对比 | ✅ 已实现 |
| **P12 历史数据扩展** | P1 | 8季度历史数据 (2024-Q1 ~ 2025-Q4) | ✅ 已实现 |
| **P13 Y轴自适应** | P2 | 图表Y轴根据数据范围自动调整 | ✅ 已实现 |

#### P11 收入vs成本趋势对比详情

- **展示位置**: Dashboard HeroCard 下方，核心对比区域
- **API端点**: `/api/v1/financials/growth-comparison`
- **数据来源**: `inference_coverage.yml` (OpenAI + Anthropic 季度数据)
- **核心指标定义**:

| 指标 | 定义 | 计算公式 |
|------|------|----------|
| **收入增速** | 推理收入季度环比增长率 | `(本季收入 - 上季收入) / 上季收入 × 100%` |
| **成本增速** | AI资产折旧季度环比增长率 | `(本季折旧 - 上季折旧) / 上季折旧 × 100%` |
| **净差值** | 收入增速 - 成本增速 | 正值 = 挣钱比花钱快 |
...
| v1.3 | 2026-01-30 | 新增 Phase 4: 收入vs成本趋势对比卡片 (P11)，历史数据扩展至8季度，Y轴自适应 |
